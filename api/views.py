...
class JoinRoomView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if code != None:
            ...
            return Response({'Bad Rquest': 'Invalid room code.'}, status=status.HTTP_400_BAD_REQUEST)
        return Response({'Bad Rquest': 'Invalid post data, did not find a code key.'}, status=status.HTTP_400_BAD_REQUEST)

class CheckIfUserInRoomView(APIView):
    def get(self, request, format=None):
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
